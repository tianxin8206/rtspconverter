...
namespace RtspConverter.FFmpeg
{
    public unsafe class VideoConvert
    {
        ...
        static int OpenInputFile(string filename)
        {
            ...
            {
                if ((ret = ffmpeg.avformat_open_input(ps, filename, null, null)) < 0)
                {
        ...
        static int OpenOutputFile(string filename)
        {
            ...
            {
                ffmpeg.avformat_alloc_output_context2(ctx, null, "hls", filename);
            }
            ...
            for (int i = 0; i < ifmt_ctx->nb_streams; i++)
            {
                ...
                if (dec_ctx->codec_type == AVMediaType.AVMEDIA_TYPE_VIDEO)
                {
                    ...
                    // av_opt_set_int(ofmt_ctx->priv_data, "hls_list_size", 10, AV_OPT_SEARCH_CHILDREN);
                }
        ...
        static int encode_write_frame(AVFrame* filt_frame, int stream_index, int* got_frame)
        {
            ...
            ret = enc_func(stream_index, ofmt_ctx->streams[stream_index]->codec, &enc_pkt, filt_frame, got_frame);
            if (ret < 0)
                return ret;
        ...
        int main(int argc, char** argv)
        {
            ...
            if ((ret = OpenInputFile("test.flv")) < 0)
                goto end;
            if ((ret = OpenOutputFile("test.m3u8")) < 0)
                goto end;
            aacbsfc = ffmpeg.av_bitstream_filter_init("aac_adtstoasc");
            /* read all packets */
            int count = 0;
            int flag = 1;
            while (true)
            {
                ...
                ret = dec_func(type, ifmt_ctx->streams[stream_index]->codec, frame, &got_frame, &packet);
                if (ret < 0)
                {
            ...
            ffmpeg.av_free_packet(&packet);
            ffmpeg.av_frame_free(&frame);
            ffmpeg.av_bitstream_filter_close(aacbsfc);
            for (i = 0; i < ifmt_ctx->nb_streams; i++)
            {
